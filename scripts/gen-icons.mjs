import sharp from 'sharp'
import { writeFileSync } from 'node:fs'

// Lucide "carrot" paths (viewBox 0 0 24 24), drawn in cream on a tomato tile.
const CARROT = `
  <g transform="translate(124,124) scale(11)" fill="none" stroke="#F6EFE2"
     stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
    <path d="M2.27 21.7s9.87-3.5 12.73-6.36a4.5 4.5 0 0 0-6.36-6.37C5.77 11.84 2.27 21.7 2.27 21.7zM8.64 14l-2.05-2.04M15.34 15l-2.46-2.46"/>
    <path d="M22 9s-1.33-2-3.5-2C16.86 7 15 9 15 9s1.33 2 3.5 2S22 9 22 9z"/>
    <path d="M15 2s-2 1.33-2 3.5S15 9 15 9s2-1.84 2-3.5C17 3.33 15 2 15 2z"/>
  </g>`

const svg = (rx) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
     <rect width="512" height="512" rx="${rx}" fill="#FF5A3C"/>${CARROT}
   </svg>`

const rounded = Buffer.from(svg(112))
const square = Buffer.from(svg(0))

await sharp(rounded).resize(512, 512).png().toFile('public/pwa-512.png')
await sharp(rounded).resize(192, 192).png().toFile('public/pwa-192.png')
await sharp(rounded).resize(512, 512).png().toFile('public/pwa-maskable-512.png')
await sharp(square).resize(180, 180).png().toFile('public/apple-touch-icon.png')
writeFileSync('public/favicon.svg', svg(112))

console.log('✓ Icônes PWA générées dans public/')
